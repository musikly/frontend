...
async function getRandomSongDetails(req, res) {
    try {
        ...
        const songResponse = await axios.get('http://backend:8080/api/v1/music/random');
        const songUrl = songResponse.data;


        // Fetch artwork and stream URLs for the song
        const artworkResponse = await axios.get(`http://backend:8080/api/v1/music/artwork?url=${songUrl}`);
        const streamResponse = await axios.get(`http://backend:8080/api/v1/music/stream?url=${songUrl}`);
